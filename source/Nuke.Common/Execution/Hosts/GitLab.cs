...
namespace Nuke.Common.Execution.Hosts
{
    ...
    public class GitLab : Host
    {
        ...
        /// </summary>
        public string CommitRefName => EnvironmentInfo.Variable("CI_COMMIT_REF_NAME");
        ...
        /// </summary>
        public string CommitRefSlug => EnvironmentInfo.Variable("CI_COMMIT_REF_SLUG");

        /// <summary>
        /// The commit revision for which project is built.
        /// </summary>
        public string CommitSha => EnvironmentInfo.Variable("CI_COMMIT_SHA");

        /// <summary>
        /// The commit tag name. Present only when building tags.
        /// </summary>
        public string CommitTag => EnvironmentInfo.Variable("CI_COMMIT_TAG");

        /// <summary>
        /// The path to CI config file. Defaults to <c>.gitlab-ci.yml</c>.
        /// </summary>
        public string ConfigPath => EnvironmentInfo.EnsureVariable("CI_CONFIG_PATH");

        /// <summary>
        /// Whether <a href="https://docs.gitlab.com/ce/ci/variables/README.html#debug-tracing">debug tracing</a> is enabled.
        /// </summary>
        public bool DebugTrace => EnvironmentInfo.Variable<bool>("CI_DEBUG_TRACE");
        ...
        /// </summary>
        public bool DisposableEnvironment => EnvironmentInfo.Variable<bool>("CI_DISPOSABLE_ENVIRONMENT");

        /// <summary>
        /// The name of the environment for this job.
        /// </summary>
        public string EnvironmentName => EnvironmentInfo.Variable("CI_ENVIRONMENT_NAME");

        /// <summary>
        /// A simplified version of the environment name, suitable for inclusion in DNS, URLs, Kubernetes labels, etc.
        /// </summary>
        public string EnvironmentSlug => EnvironmentInfo.Variable("CI_ENVIRONMENT_SLUG");

        /// <summary>
        /// The URL of the environment for this job.
        /// </summary>
        public string EnvironmentUrl => EnvironmentInfo.Variable("CI_ENVIRONMENT_URL");

        /// <summary>
        /// The unique id of the current job that GitLab CI uses internally.
        /// </summary>
        public string JobId => EnvironmentInfo.Variable("CI_JOB_ID");

        /// <summary>
        /// The flag to indicate that job was manually started.
        /// </summary>
        public bool JobManual => EnvironmentInfo.Variable<bool>("CI_JOB_MANUAL");

        /// <summary>
        /// The name of the job as defined in <c>.gitlab-ci.yml</c>.
        /// </summary>
        public string JobName => EnvironmentInfo.Variable("CI_JOB_NAME");

        /// <summary>
        /// The name of the stage as defined in <c>.gitlab-ci.yml</c>.
        /// </summary>
        public string JobStage => EnvironmentInfo.Variable("CI_JOB_STAGE");

        /// <summary>
        /// Token used for authenticating with the GitLab Container Registry.
        /// </summary>
        public string JobToken => EnvironmentInfo.Variable("CI_JOB_TOKEN");

        /// <summary>
        /// The URL to clone the Git repository.
        /// </summary>
        public string RepositoryUrl => EnvironmentInfo.Variable("CI_REPOSITORY_URL");

        /// <summary>
        /// The description of the runner as saved in GitLab.
        /// </summary>
        public string RunnerDescription => EnvironmentInfo.Variable("CI_RUNNER_DESCRIPTION");

        /// <summary>
        /// The unique id of runner being used.
        /// </summary>
        public string RunnerId => EnvironmentInfo.Variable("CI_RUNNER_ID");

        /// <summary>
        /// The defined runner tags.
        /// </summary>
        public string RunnerTags => EnvironmentInfo.Variable("CI_RUNNER_TAGS");

        /// <summary>
        /// The unique id of the current pipeline that GitLab CI uses internally.
        /// </summary>
        public string PipelineId => EnvironmentInfo.Variable("CI_PIPELINE_ID");

        /// <summary>
        /// The flag to indicate that job was <a href="https://docs.gitlab.com/ce/ci/triggers/README.html">triggered</a>.
        /// </summary>
        public bool PipelineTriggered => EnvironmentInfo.Variable<bool>("CI_PIPELINE_TRIGGERED");
        ...
        /// </summary>
        public string PipelineSource => EnvironmentInfo.Variable("CI_PIPELINE_SOURCE");

        /// <summary>
        /// The full path where the repository is cloned and where the job is run.
        /// </summary>
        public string ProjectDir => EnvironmentInfo.Variable("CI_PROJECT_DIR");

        /// <summary>
        /// The unique id of the current project that GitLab CI uses internally.
        /// </summary>
        public string ProjectId => EnvironmentInfo.Variable("CI_PROJECT_ID");

        /// <summary>
        /// The project name that is currently being built (actually it is project folder name).
        /// </summary>
        public string ProjectName => EnvironmentInfo.Variable("CI_PROJECT_NAME");

        /// <summary>
        /// The project namespace (username or groupname) that is currently being built.
        /// </summary>
        public string ProjectNamespace => EnvironmentInfo.Variable("CI_PROJECT_NAMESPACE");

        /// <summary>
        /// The namespace with project name.
        /// </summary>
        public string ProjectPath => EnvironmentInfo.Variable("CI_PROJECT_PATH");
        ...
        /// </summary>
        public string ProjectPathSlug => EnvironmentInfo.Variable("CI_PROJECT_PATH_SLUG");

        /// <summary>
        /// The HTTP address to access project.
        /// </summary>
        public string ProjectUrl => EnvironmentInfo.Variable("CI_PROJECT_URL");

        /// <summary>
        /// The project visibility (internal, private, public).
        /// </summary>
        public GitLabProjectVisibility ProjectVisibility => EnvironmentInfo.Variable<GitLabProjectVisibility>("CI_PROJECT_VISIBILITY");

        /// <summary>
        /// If the Container Registry is enabled it returns the address of GitLab's Container Registry.
        /// </summary>
        public string Registry => EnvironmentInfo.Variable("CI_REGISTRY");

        /// <summary>
        /// If the Container Registry is enabled for the project it returns the address of the registry tied to the specific project.
        /// </summary>
        public string RegistryImage => EnvironmentInfo.Variable("CI_REGISTRY_IMAGE");

        /// <summary>
        /// The password to use to push containers to the GitLab Container Registry.
        /// </summary>
        public string RegistryPassword => EnvironmentInfo.Variable("CI_REGISTRY_PASSWORD");

        /// <summary>
        /// The username to use to push containers to the GitLab Container Registry.
        /// </summary>
        public string RegistryUser => EnvironmentInfo.Variable("CI_REGISTRY_USER");

        /// <summary>
        /// Mark that job is executed in CI environment.
        /// </summary>
        public bool Server => EnvironmentInfo.Variable<bool>("CI_SERVER");

        /// <summary>
        /// The name of CI server that is used to coordinate jobs.
        /// </summary>
        public string ServerName => EnvironmentInfo.Variable("CI_SERVER_NAME");

        /// <summary>
        /// GitLab revision that is used to schedule jobs.
        /// </summary>
        public string ServerRevision => EnvironmentInfo.Variable("CI_SERVER_REVISION");

        /// <summary>
        /// GitLab version that is used to schedule jobs.
        /// </summary>
        public string ServerVersion => EnvironmentInfo.Variable("CI_SERVER_VERSION");
        ...
        /// </summary>
        public bool SharedEnvironment => EnvironmentInfo.Variable<bool>("CI_SHARED_ENVIRONMENT");

        /// <summary>
        /// Number of attempts to download artifacts running a job.
        /// </summary>
        public int ArtifactDownloadAttempts => EnvironmentInfo.Variable<int>("ARTIFACT_DOWNLOAD_ATTEMPTS");

        /// <summary>
        /// Number of attempts to fetch sources running a job.
        /// </summary>
        public int GetSourcesAttempts => EnvironmentInfo.Variable<int>("GET_SOURCES_ATTEMPTS");

        /// <summary>
        /// Mark that job is executed in GitLab CI environment.
        /// </summary>
        public bool GitLabCi => EnvironmentInfo.Variable<bool>("GITLAB_CI");

        /// <summary>
        /// The id of the user who started the job.
        /// </summary>
        public string GitLabUserId => EnvironmentInfo.Variable("GITLAB_USER_ID");

        /// <summary>
        /// The email of the user who started the job.
        /// </summary>
        public string GitLabUserEmail => EnvironmentInfo.Variable("GITLAB_USER_EMAIL");

        /// <summary>
        /// The login username of the user who started the job.
        /// </summary>
        public string GitLabUserLogin => EnvironmentInfo.Variable("GITLAB_USER_LOGIN");

        /// <summary>
        /// The real name of the user who started the job.
        /// </summary>
        public string GitLabUserName => EnvironmentInfo.Variable("GITLAB_USER_NAME");

        /// <summary>
        /// Number of attempts to restore the cache running a job.
        /// </summary>
        public int RestoreCacheAttempts => EnvironmentInfo.Variable<int>("RESTORE_CACHE_ATTEMPTS");
    }
}
