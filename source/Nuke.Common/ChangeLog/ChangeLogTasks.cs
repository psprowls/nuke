...
namespace Nuke.Common.ChangeLog
{
    ...
    public static class ChangelogTasks
    {
        ...
        {
            Logger.Info($"Finalizing {PathUtility.GetRootRelativePath(changelogFile.Path)} for '{tag}'...");
        ...
        {
            Logger.Info($"Finalizing {PathUtility.GetRootRelativePath(changelogFile)} for '{tag}'...");
